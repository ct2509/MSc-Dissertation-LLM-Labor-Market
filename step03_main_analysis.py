...
step01_build_panel.py. It does not reread the raw ACS files.
...
9. Joint 2023/2024 significance tests for the main hypotheses.
10. Original M retained only as a robustness measure.
...
# Only these outcomes appear in the compact dissertation main table.
# Employment share is still estimated and saved as an appendix robustness result.
MAIN_TABLE_OUTCOMES = {"wage", "employment"}
...
def load_panel() -> tuple[pd.DataFrame, pd.DataFrame]:
    if not PANEL_FILE.exists():
        raise FileNotFoundError(
            ...
            "Run step01_build_panel.py first and keep its output in this folder."
        )
...
def add_soc_year_deviations(
    ...
    Occupation fixed effects already absorb permanent SOC-group differences,
    and year fixed effects absorb aggregate annual shocks. Therefore, only
    non-reference SOC groups interacted with post years are required.

    The first SOC major group is used as the reference group.
    The reference year is 2022.
    """
    data = data.copy()
...
def fit_fe_model(
    ...
    # Add non-redundant SOC-major-group-by-year deviations.
    model_data, soc_year_cols = add_soc_year_deviations(model_data)
        ...
        rows.append(
            {
                ...
                "occupation_clusters": (
                    model_data["acs_occ_code"].nunique()
                ),
...
def joint_zero_test(
    ...
    """Jointly test whether all listed coefficients equal zero."""

    missing = [term for term in terms if term not in result.params.index]
    if missing:
        raise KeyError(
            f"Cannot run {test_name}; missing coefficients: {missing}"
        )
    ...
    return {
        ...
        "outcome": outcome,
        "model": model_name,
        "scheme": scheme or "not_applicable",
        "tested_terms": ";".join(terms),
        "null_hypothesis": restriction,
        "number_of_restrictions": len(terms),
...
def run_continuous_models(
    ...
    for outcome_name, settings in OUTCOMES.items():
        ...
        dynamic_model_name = f"{outcome_name}_preferred_dynamic"
        dynamic_result, rows = fit_fe_model(
            data=panel,
            outcome=outcome,
            terms=dynamic_terms,
        ...
        for test_name, terms in joint_test_terms.items():
            joint_rows.append(
                joint_zero_test(
    ...
    return pooled_table, dynamic_table, joint_table
...
def run_adjusted_category_models(
    ...
    for scheme_name, settings in CATEGORY_SCHEMES.items():
        ...
        for outcome_name, outcome_settings in OUTCOMES.items():
            ...
            # The main joint HEHC-versus-HELC test uses only the preferred,
            # pre-specified baseline classification.
            if scheme_name == "baseline_within_highE_median":
                joint_rows.append(
                    joint_zero_test(
                        result=dynamic_result,
                        terms=["high_c_y2023", "high_c_y2024"],
                        test_name="HEHC_minus_HELC_2023_2024_joint",
                        outcome_name=outcome_name,
                        outcome=outcome,
                        model_name=dynamic_model_name,
                        analysis="exposure_adjusted_category_dynamic",
                        scheme=scheme_name,
                    )
                )
...
def build_compact_main_table(
    ...
    # The dissertation main table reports only wage and employment.
    # Employment share remains in the detailed robustness outputs.
    pooled = pooled_results.loc[
        pooled_results["outcome_name"].isin(MAIN_TABLE_OUTCOMES)
    ].copy()
    pooled["analysis"] = "continuous_pooled"
    pooled["specification"] = pooled["term"]

    dynamic = dynamic_results.loc[
        dynamic_results["outcome_name"].isin(MAIN_TABLE_OUTCOMES)
    ].copy()
    dynamic["analysis"] = "continuous_dynamic"
    dynamic["specification"] = dynamic["term"]

    category = category_results.loc[
        category_results["outcome_name"].isin(MAIN_TABLE_OUTCOMES)
        & category_results["scheme"].eq("baseline_within_highE_median")
...
def main() -> None:
    ...
    (
        pooled_results,
        dynamic_results,
        continuous_joint_tests,
    ) = run_continuous_models(panel)
    ...
    (
        category_counts,
        category_results,
        category_joint_tests,
    ) = run_adjusted_category_models(panel)
    ...
    # Four joint tests per outcome:
    # 1. Exposure in 2023 and 2024
    # 2. Complementarity in 2023 and 2024
    # 3. E x C interaction in 2023 and 2024
    # 4. Exposure-adjusted HEHC minus HELC in 2023 and 2024
    joint_tests = pd.concat(
        [continuous_joint_tests, category_joint_tests],
        ignore_index=True,
    )
    joint_tests = add_fdr(
        joint_tests,
    ...
    joint_tests["main_table_outcome"] = joint_tests[
        "outcome_name"
    ].isin(MAIN_TABLE_OUTCOMES)
    joint_tests.to_csv(
        out("joint_significance_tests.csv"),
        index=False,
    )
    ...
    print("  6. out_step3_joint_significance_tests.csv")
    print("  7. out_step3_M_robustness_results.csv")
    print("  8. out_step3_dissertation_main_table.csv")
    print("  9. out_step3_*_preferred_dynamic.png")
    print(" 10. out_step3_*_adjusted_HEHC_HELC.png")
